"""Training-only shrinkage toward causal immediate-score calibration."""
...
def main() -> None:
    ...
    calibrated = causal_isotonic(data, score, "points")
    ...
    for index, share in enumerate(SHARES, start=1):
        local_score = (1 - share) * score + share * calibrated
        print(f"Running {index}/{len(SHARES)}: calibration share {share:.2f}", flush=True)
    ...
    result = {
        "status": "training-selected causal shrinkage",
        "selected": selected,
        "experiments": rows,
        "selectedLift": round(selected["evaluationAverage"] - rows[0]["evaluationAverage"], 1),
    }
    output = lens.ROOT / "analysis" / "data" / "immediate_calibration_shrinkage.json"
